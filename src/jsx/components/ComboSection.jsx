...
class ComboSection extends Component {
    ...
    render() {
        ...
        // Loop through each launcher to create HTML blocks for each launcher
        for (let i = 0; i < comboData.length; i++) {

            // Launcher header
            // Only adds hyperlink if there is a combo launcher demo
            returnHTML.push (
                <h3 key={`launcher-${i}`}>
            ...
            for (let x = 0; x < comboData[i].routes.length; x++) {
                // Format the combo notation to give the proper CSS classes to the displayed notation
